...
# creates grid based on obstacle locations  
def createGrid(app):
    app.grid = []
    ...
    for row in range(app.rows):
        ...
        for col in range(app.cols):
            ...
            for zapper in app.zapperList:
                ...
                if (checkCircleRectOverlap(zapper.radius, firstZapX, firstZapY, left, top, left + cellWidth, top + cellHeight) or
                    checkCircleRectOverlap(zapper.radius, secondZapX, secondZapY, left, top, left + cellWidth, top + cellHeight)):
                    rowList.append(1)
                    continue
                firstZap = Point(firstZapX, firstZapY)
                secondZap = Point(secondZapX, secondZapY)
                Rect1 = Point(left, top)
                Rect2 = Point(left + cellWidth, top)
                Rect3 = Point(left + cellWidth, top + cellHeight)
                Rect4 = Point(left, top + cellHeight)
                if (doIntersect(firstZap, secondZap, Rect1, Rect2) or doIntersect(firstZap, secondZap, Rect2, Rect3)
                    or doIntersect(firstZap, secondZap, Rect3, Rect4) or doIntersect(firstZap, secondZap, Rect4, Rect1)):
                    rowList.append(1)
                    continue

            for missile in app.missileList:
                missileX = missile.coords[0] + 8
                missileY = missile.coords[1] + 15
                if missileX == app.width:
                    continue
                if checkRectRectOverlap(missileX, missileY, left, top, missileX + missile.width, missileY + missile.height, left + cellWidth, top + cellHeight):
                    rowList.append(1)
                    print(len(rowList))
                    continue
...
def pathFinding(app, currentRow, currentCol, L):
    if currentCol >= 39:
        return L
    else:
        for drow, dcol in ((0, 1), (-1, 0), (1 , 0)):
            if currentRow + drow < 0 or currentRow + drow >= app.rows or currentCol + dcol < 0 or currentCol + dcol >= app.cols:
                continue
            if app.grid[currentRow + drow][currentCol + dcol] == 0:
                L.append((currentRow + drow, currentCol + dcol))
                result = pathFinding(app, currentRow + drow, currentCol + dcol, L)
                if result != None:
                    return result
                L.pop()
...
def checkCircleRectOverlap(R, Xc, Yc, X1, Y1, X2, Y2):
...
def checkRectRectOverlap(ax1, ay1, bx1, by1, ax2, ay2, bx2, by2):
...
def onMousePress(app, mouseX, mouseY):
    print(getCell(app, app.playerX, app.playerY))
...
def drawPath(app):
    if app.possiblePath == None:
        return
    for row, col in app.possiblePath:
        left, top, cellWidth, cellHeight = getCellBounds(app, row, col)
        drawCircle(left + cellWidth / 2, top + cellHeight / 2, 3)
...
def stepEvents(app):
    ...
    if app.pathfind:
        createGrid(app)
        currentRow, currentCol = getCell(app, app.playerX, app.playerY)
        app.possiblePath = (pathFinding(app, int(currentRow), int(currentCol), []))
        if app.possiblePath == None:
            app.pathfind = False
